...
void printMenu(void)
{
    ...
    printf("  * not available for E-Magnetic Driver channels\n");
    printf("[q] quit\n");
}
...
int main()
{
    ...
    // The returned device list contains the locators of all found MCS2 devices separated by
    // newlines. The "ioDeviceListLen" parameter must contain the size of the passed deviceList
    // buffer and contains the length of the deviceList when the function returns. In case the
    // deviceList size exceed the provided buffer size an "SA_CTL_QUERYBUFFER_SIZE_ERROR" is
    // returned.
    ...
    // Both methods are not show in this example. Instead, refer to the further programming
    // examples.
    ...
    if (type == SA_CTL_STICK_SLIP_PIEZO_DRIVER)
    {
        ...
        // A value of 0 deactivates the hold time feature, a value of SA_CTL_INFINITE (0xffffffff)
        // sets the time to infinite. (Until manually stopped by "SA_CTL_Stop") Here we set the hold
        // time to 1000 ms.
    ...
    do
    {
        ...
        switch (key)
        {
            ...
            case 'p':
                result =
                    SA_CTL_GetProperty_i64(dHandle, channel, SA_CTL_PKEY_POSITION, &position, 0);
                exitOnError(result);
    ...
    // Before closing the program the connection to the device must be closed by calling
    // "SA_CTL_Close".
    SA_CTL_Close(dHandle);
    printf("MCS2 close.\n");
    printf("*******************************************************\n");
...
// CALIBRATION
// The calibration sequence is used to increase the precision of the position calculation. This
// function should be called once for each channel if the mechanical setup changes. (e.g. a
// different positioner was connected to the channel, the positioner type was set to a different
// type) The calibration data will be saved to non-volatile memory, thus it is not necessary to
// perform the calibration sequence on each initialization. Note: the
// "SA_CTL_CH_STATE_BIT_IS_CALIBRATED" in the channel state can be used to determine if valid
// calibration data is stored for the specific channel.

// During the calibration sequence the positioner performs a movement of up to several mm, make sure
// to not start the calibration near a mechanical endstop in order to ensure proper operation. See
// the MCS2 Programmers Guide for more information on the calibration.
void calibrate(int8_t channel)
{
    SA_CTL_Result_t result;
    ...
    // Note that the function call returns immediately, without waiting for the movement to
    // complete. The "SA_CTL_CH_STATE_BIT_CALIBRATING" in the channel state can be monitored to
    // determine the end of the calibration sequence.
    exitOnError(result);
}
...
void findReference(int8_t channel)
{
    ...
    // Note that the function call returns immediately, without waiting for the movement to
    // complete. The "SA_CTL_CH_STATE_BIT_REFERENCING" in the channel state can be monitored to
    // determine the end of the referencing sequence.
    exitOnError(result);
}
...
// It can be a position value (in case of closed loop movement mode), a scan value (in case of scan
// move mode) or a number of steps (in case of step move mode).
void move(int8_t channel, int32_t moveMode, int8_t direction)
{
    SA_CTL_Result_t result;
    int64_t moveValue;
    ...
    switch (moveMode)
    {
        case SA_CTL_MOVE_MODE_CL_ABSOLUTE:
            ...
            result =
                SA_CTL_SetProperty_i64(dHandle, channel, SA_CTL_PKEY_MOVE_VELOCITY, 1000000000);
            exitOnError(result);
            // Set move acceleration [in pm/s2].
            result =
                SA_CTL_SetProperty_i64(dHandle, channel, SA_CTL_PKEY_MOVE_ACCELERATION, 1000000000);
            exitOnError(result);
            // Specify absolute position [in pm].
            // (For Piezo Scanner channels adjust to valid value within move range, e.g.
            // +-10000000.)
            if (direction)
                moveValue = -2000000000;
            else
                moveValue = 1000000000;
            printf(
                "MCS2 move channel %d to absolute position: %" PRId64 " pm.\n",
                channel,
                moveValue
            );
            break;
        ...
        case SA_CTL_MOVE_MODE_SCAN_ABSOLUTE:
            ...
            result =
                SA_CTL_SetProperty_i64(dHandle, channel, SA_CTL_PKEY_SCAN_VELOCITY, (65535 * 2));
            exitOnError(result);
        ...
        case SA_CTL_MOVE_MODE_SCAN_RELATIVE:
            ...
            // Valid range( Stick-Slip Piezo): -65535 to 65535 corresponding to 0 to 100V piezo
            // voltage Valid range (Piezo Scanner): -1048575 to 1048575 corresponding to -20 to 100V
            // piezo voltage If the resulting absolute scan target exceeds the valid range the scan
            // movement will stop at the boundary.
            moveValue = 65535;
            if (direction)
                moveValue = -moveValue;
            printf("MCS2 scan channel %d relative: %" PRId64 ".\n", channel, moveValue);
            break;
        case SA_CTL_MOVE_MODE_STEP: // (only Stick-Slip piezo positioners)
            ...
            if (direction)
                moveValue = -moveValue;
            printf(
                "MCS2 open loop step move, channel %d, steps: %" PRId64 ".\n",
                channel,
                moveValue
            );
            break;
    ...
    // Note that the function call returns immediately, without waiting for the movement to
    // complete. The "SA_CTL_CH_STATE_BIT_ACTIVELY_MOVING" (and
    // "SA_CTL_CH_STATE_BIT_CLOSED_LOOP_ACTIVE") in the channel state can be monitored to determine
    // the end of the movement.
    exitOnError(result);
}

// STOP
// This command stops any ongoing movement. It also stops the hold position feature of a closed loop
// command. Note for closed loop movements with acceleration control enabled: The first "stop"
// command sent while moving triggers the positioner to come to a halt by decelerating to zero. A
// second "stop" command triggers a hard stop ("emergency stop").
void stop(int8_t channel)
{
    SA_CTL_Result_t result;
