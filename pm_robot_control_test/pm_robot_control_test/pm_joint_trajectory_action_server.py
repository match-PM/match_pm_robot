...
class PMJointTrajectoryActionServer(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("PMJointTrajectoryActionServer has been initialised")
    ...
    def execute_callback(self, goal_handle):
        ...
        self.get_logger().info(f'Targets: {self.goal.targets}')
        ...
        while not all(goal_reached):
            for idx, joint in enumerate(self.joint_names):
                ...
                lower_threshold = (self.goal.targets[idx]-10**-5)#*0.95 
                upper_threshold = (self.goal.targets[idx]+10**-5)#*1.05
...
def main(args=None):
    ...
    try:
        pm_joint_trajectory_action_server = PMJointTrajectoryActionServer()

        executor = MultiThreadedExecutor(num_threads=2)
        executor.add_node(pm_joint_trajectory_action_server)

        try:
            executor.spin()
        finally:
            executor.shutdown()
            pm_joint_trajectory_action_server.destroy_node()
