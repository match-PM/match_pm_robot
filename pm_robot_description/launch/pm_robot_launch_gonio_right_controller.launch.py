...
def generate_launch_description():
    ...
    Spawn_pm_robot_gonio_right_JTC = Node(
        package='controller_manager',
        executable='spawner',
        arguments=[
            "pm_robot_gonio_right_JTC",
            "--controller-manager",
            "/controller_manager",
        ],
    )
    ...
    delay_Spawn_pm_robot_gonio_right_JTC = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=Spawn_pm_robot_gonio_right_JSB,
            on_exit=[Spawn_pm_robot_gonio_right_JTC],
        )
    )
    ...
    ld.add_action(delay_Spawn_pm_robot_gonio_right_JTC)
    return ld
